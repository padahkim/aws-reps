...
## 범위 — v1은 4챕터에서 닫았고, 이후 챕터를 추가하고 있다

v1이 공개한 챕터는 **4개**였다. `ch0-1 AWS 기초` · `ch0-2 IAM` · `ch1-1 S3` · `ch1-2 Lambda`.

설계된 커리큘럼([`docs/CURRICULUM.md`](docs/CURRICULUM.md))은 24챕터짜리 트리다. 전부 채울 때까지 첫 버전을 미루는 대신, 네 챕터로 학습 구조를 완성한 시점에 **v1의 범위를 확정했다.** 이후 챕터는 원본 초안을 규약에 맞게 변환하고 검수해 1~2개 단위로 추가 공개하고 있다.
...
그래서 **v1은 4챕터에서 닫았지만, 그 숫자를 프로젝트의 영구 상한으로 삼지는 않았다.** 후속 챕터도 v1에서 세운 같은 품질 기준을 통과한 것부터 차례로 더한다.

---

## 학습 설계 — "읽었다"와 "안다"를 구분한다

기술 문서를 눈으로 훑으면 안다는 착각이 남는다. 그래서 학습 흐름 곳곳에 **답을 떠올려 보게 하는 장치**를 뒀다. 모두 학습을 돕는 프롬프트이며, 답해야만 다음으로 넘어가는 관문은 아니다.
...
| **오답 노트** | 상시 | 챕터 퀴즈에서 틀린 문항을 **Leitner 3상자**로 굴린다. 복습 큐는 기한이 된 것만 내고, 별도의 **자유 연습 모드**로 졸업한 문항까지 범위를 골라 더 풀 수 있다 |
...
| **콘텐츠 ↔ 앱 분리** | 앱(`app/`)은 셸이고 학습 내용은 `content/`의 챕터 모듈이다. 데이터 통로는 `lib/content.ts` 하나이고, MDX 렌더 팔레트만 루트 `mdx-components.tsx`가 챕터 UI를 직접 연결한다. 앱은 본문의 내부 표현을 모른 채 규약이 내보내는 것만 소비한다 |
...
`develop` 대상 PR과 `develop` push마다 [CI](.github/workflows/ci.yml)가 돌린다:
...
v1 범위 밖에서 이어 가는 것들. 설계와 조사는 리포와 [이슈 트래커](https://github.com/padahkim/aws-reps/issues)에 남아 있다.

- **후속 커리큘럼** — 원본 초안이 있는 챕터부터 규약에 맞춰 1~2개 단위로 변환·검수해 추가 공개한다
- **AWS 프로덕션 이전** — 실 도메인 + S3/CloudFront(OAC) + SAM 템플릿 + GitHub Actions OIDC 배포
- **계정 연동** — 로그인, 서버 귀속 학습 데이터, 기기 간 동기화
- **AI 기능** — 질문·해설, 오답 분석, 문제 생성

학습 루프와 앱 구조의 초기 설계는 [`docs/design/`](docs/design)에, 각 후속 계획의 조사와 결정은 해당 spike 이슈에 있다.
