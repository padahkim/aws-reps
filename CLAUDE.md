...
- 과제 관리는 **GitHub Issues가 단일 진실**이다. 새 과제·버그·개선점은 발견 즉시 이슈로 등록한다 — 별도 백로그 md 금지.
- 이슈 형식: 제목 + 본문 완료 기준 체크박스. 그 이상 금지 — 라벨·템플릿·마일스톤·프로젝트 보드는 같은 필요가 3번 반복 증명되기 전까지 도입하지 않는다.
