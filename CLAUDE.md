...
- **Projects 보드 "aws-reps 로드맵"(프로젝트 #1)** 은 큐의 시각화다. 갱신은 사람이 아니라 세션이 한다: 이슈 착수 시 In Progress, `/land` 착지 시 Done (절차는 land 스킬). 어긋난 보드는 없는 보드보다 나쁘다 — gh 차단 머신에서 작업했다면 다음 gh 가능 세션 시작 시 보드를 현실에 맞춘다.
