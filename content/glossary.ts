...
export const glossary: GlossaryTerm[] = [
  ...
  },
  {
    ...
    detail:
      "실행 환경 수명 주기의 INIT 단계에서 일어난다 — 런타임을 띄우고 배포 패키지를 내려받아 핸들러 바깥의 초기화 코드까지 돌리는 시간이다. 뒤이은 호출이 그 데워진 환경에 도착하면 이 비용을 다시 내지 않는다 — 다만 재사용은 보장이 아니다. 동시 호출이 늘어 환경이 더 필요해지거나, 오래 놀아 환경이 회수되거나, 함수 설정을 바꾸면 새 환경이 떠서 다시 겪는다.\n\n첫 요청 자체를 짧게 하는 수단은 둘이다 — 프로비저닝된 동시성으로 환경을 미리 초기화해 두거나, 배포 패키지·의존성을 줄여 로드를 짧게 하는 것이다.\n\nSDK 클라이언트·DB 연결을 핸들러 밖에서 한 번만 만드는 관행은 결이 다르다. 그 작업도 INIT에서 비용을 내므로 첫 호출은 여전히 느리고, 대신 뒤이은 웜 호출이 그것을 재사용해 빨라진다 — 시험에서 자주 정답이 되는 것도 이 웜 경로 개선이다.",
    chapterId: "ch1-2",
  },
  {
    id: "concurrency",
  ...
  },
  {
    ...
    detail:
      "층이 셋이라 헷갈리기 쉽다. Lambda 비동기 호출의 DLQ는 재시도까지 실패한 이벤트를 SQS 큐나 SNS 토픽으로 보낸다 — 함수 오류는 기본 2회 재시도한 뒤 여기로 오고, 스로틀·시스템 오류는 이벤트 수명(기본 6시간) 동안 지수 백오프로 계속 다시 시도한 뒤에 온다 — 본문은 원래 이벤트이고 오류 쪽은 RequestID·ErrorCode·ErrorMessage가 메시지 속성으로 붙는 정도라, 함수의 응답까지 담은 구조화된 호출 기록은 없다.\n\n같은 자리를 대체하는 최신 수단이 Destinations다. 실패만이 아니라 성공도 보낼 수 있고 호출 응답·오류 정보까지 함께 실어 주므로, 새로 만든다면 이쪽이 권장 경로다.\n\nSQS 큐 자체의 DLQ는 층이 다르다 — 큐에 건 리드라이브 정책의 maxReceiveCount만큼 소비에 실패한 메시지를 다른 큐로 옮기는 것으로, Lambda 설정이 아니라 큐 설정이다.",
    chapterId: "ch1-2",
  },
  {
    id: "destinations",
  ...
  },
  {
    ...
    detail:
      "템플릿 맨 위의 Transform 선언이 SAM 문법을 CloudFormation으로 펼친다 — 배포되는 실체는 결국 CloudFormation 스택이다. AWS::Serverless::Function 한 덩어리가 함수를 만들고, Role을 따로 지정하지 않았으면 실행 역할까지, Events에 적은 만큼 트리거까지 함께 만들어 준다 — 로그 그룹은 여기 없다(Lambda가 첫 호출 때 런타임에 만든다).\n\nsam build로 의존성을 묶고 sam deploy로 배포하며, sam local invoke·sam local start-api는 도커로 함수를 로컬에서 돌려 본다.\n\n함수에 AutoPublishAlias와 DeploymentPreference를 적으면 CodeDeploy가 붙어 별칭 트래픽을 카나리·선형으로 옮긴다 — SAM이 무중단 배포를 다루는 지점이다.",
    chapterId: "ch1-2",
  },
  {
    id: "cloudformation",
