 ...
 * §02(API·자격증명)가 이 챕터의 ★★★ 자리라 5개 중 3개가 거기서 나온다 — 섹션 균등 배분이
...
export const chapterMeta: ChapterMeta = {
  ...
  objectives: [
    ...
    "비밀번호·액세스 키·임시 자격증명을 용도로 구분하고, 애플리케이션에 줄 자격증명으로 IAM 롤을 고른다",
    "“구체적 지정이 기본값을 이긴다”는 SDK 자격증명 탐색 원칙으로 인증이 어디서 풀렸는지 짚는다 (대표 순서: 코드 → 환경변수 → 설정 파일 → 롤)",
