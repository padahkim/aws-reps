/**
 * 생성물 — 손편집 금지. `node scripts/import-drills.ts aws-basics` 재실행으로 갱신.
 * 원본: content/drills-src/aws-basics.json (11문항)
 */
...
export const quiz: Question[] = [
  // source: aws-basics-region-selection-criteria
  {
    ...
    "choices": [
      ...
      "리전 데이터센터의 정확한 물리적 주소 — AWS는 보안상 개별 데이터센터의 정확한 위치를 공개하지 않으므로, 이를 알아야 리전을 고를 수 있는 것은 아니다.",
      "필요한 서비스가 해당 리전에서 지원되는지 여부."
    ],
    "answer": [
      2
    ],
    "explanation": "리전 선택 기준은 크게 지연시간(사용자와의 거리), 규제·데이터 주권, 서비스 지원 여부, 가격으로 정리된다. 반면 개별 데이터센터의 정확한 물리적 주소는 AWS가 보안·안전상 공개하지 않는 정보라서(리전·AZ 코드로만 식별), 애초에 리전 선택에 쓸 수 있는 기준이 아니다.\n\n시험에서는 '지연시간을 줄여야 한다'는 조건이 나오면 사용자와 가까운 리전을 고르라는 신호로 읽어야 하고, '규제상 국내 보관'이 나오면 데이터 주권을 리전 선택의 결정적 근거로 봐야 한다. 참고로 한 리전 안에서 필요한 가용영역(AZ) 개수는 리전마다 다르므로(모든 리전이 최소 3개를 보장할 뿐 그 이상은 리전마다 차이가 있다), 특정 개수 이상의 AZ가 필요한 워크로드라면 AZ 개수도 실제 선택 기준이 될 수 있다.",
    "choiceExplanations": [
      "물리적 거리는 왕복 시간(RTT)에 직접 영향을 주는 실제 리전 선택 기준이다.",
      "데이터 주권·규제 준수는 리전을 결정하는 법적 요건으로 시험에서도 핵심 판단 근거로 등장한다.",
      "정답. AWS는 데이터센터의 정확한 물리적 위치를 보안상 공개하지 않는다 — 그 주소를 아는 것은 애초에 불가능하고, 필요하지도 않은 정보다.",
  ...
  // source: aws-basics-multi-az-high-availability
  {
    ...
    "choices": [
      ...
      "서로 다른 AZ에 인스턴스를 나눠 배포해 한 AZ의 장애가 전체 서비스를 멈추지 않게 한다.",
    ...
    "choiceExplanations": [
      ...
      "인스턴스 사양을 키우는 것은 처리 성능 문제이지, AZ 단위 장애로 인한 가용성 문제는 해결하지 못한다.",
  ...
  // source: aws-basics-credential-provider-chain-order
  {
    ...
    "scenario": "같은 애플리케이션 코드를 로컬 개발 환경(설정 파일 사용)과 EC2 인스턴스(IAM 역할 부착) 양쪽에서 실행한다. 코드를 전혀 수정하지 않아도 SDK가 각 환경에 맞는 자격 증명을 자동으로 찾아내는 이유를 가장 정확히 설명한 것은? (SDK·버전마다 세부 단계 구성은 다를 수 있다는 전제로 답하라)",
    "choices": [
      "SDK가 코드 실행 위치를 감지해 그 환경 전용 자격 증명 API를 별도로 호출하기 때문이다.",
      "SDK가 정해진 탐색 순서를 따라가며 처음 발견한 유효한 자격 증명을 사용하기 때문이다 — 대표적으로 코드 파라미터 → 환경변수 → 설정 파일 → 부착된 IAM 역할 순이며, 핵심 원칙은 '구체적으로 지정된 것이 기본값보다 우선한다'는 것이다.",
    ...
    "explanation": "SDK와 CLI는 정해진 순서로 여러 위치를 차례로 뒤져 자격 증명을 찾는다 — 이것이 Credential Provider Chain이다. 대표적인 예로 ① 코드에 명시된 파라미터 → ② 환경변수 → ③ 설정 파일(`~/.aws/credentials`) → ④ 붙어 있는 IAM 역할(EC2 인스턴스 프로파일, Lambda 실행 역할 등) 순서가 자주 언급되지만, AWS 공식 문서는 SDK·도구마다 실제 체인이 다르며 웹 자격 증명·IAM Identity Center·컨테이너 자격 증명 같은 단계가 더 끼어들 수 있다고 명시한다.\n\n순서에는 공통 논리가 있다 — 구체적으로 지정된 것이 기본값을 이긴다. 그래서 로컬에서는 설정 파일 단계에서, EC2에서는 역할 단계에서 자격 증명이 발견되며, 같은 코드가 두 환경 모두에서 수정 없이 동작하는 이유가 된다. 시험에서 외워야 할 것은 정확한 단계 수가 아니라 이 우선순위 원칙이다.",
    "choiceExplanations": [
      "SDK는 환경별 전용 API를 따로 호출하는 것이 아니라, 정해진 순서를 순차적으로 탐색할 뿐이다.",
      "정답. SDK는 '구체적으로 지정된 것이 기본값을 이긴다'는 원칙에 따라 정해진 순서로 자격 증명을 탐색한다 — 정확한 단계 구성·개수는 SDK·버전마다 다를 수 있다.",
      "액세스 키가 환경변수로 자동 배포되는 메커니즘은 없다 — 환경변수는 사람이나 배포 도구가 직접 설정해야 한다.",
      "체인에서 IAM 역할은 대체로 가장 마지막(기본값) 단계로 다뤄진다 — 로컬 환경에는 애초에 부착된 역할이 없으므로 그 단계까지 가지 않고 설정 파일에서 이미 발견된다."
    ],
  ...
  // source: aws-basics-403-access-denied-vs-signature-mismatch
  {
    ...
    "choices": [
      ...
      "AccessDenied는 인가 실패이고, SignatureDoesNotMatch는 요청 서명·자격 증명 문제다 — 상태 코드가 아니라 에러 코드로 원인을 구분해야 한다. 단, AccessDenied만으로 요청이 반드시 인증에 성공했다고 단정할 수는 없고(서명되지 않은 익명 요청도 AccessDenied를 받을 수 있다), 원인도 identity 기반 정책 하나로 좁힐 수 없다(리소스 기반 정책·SCP·권한 경계 등 여러 계층에서 발생 가능).",
    ...
    "explanation": "HTTP 상태 코드 403은 '거부됐다'는 사실만 알려줄 뿐 원인을 말해주지 않는다. 실제 원인은 응답에 포함된 에러 코드로 구분해야 한다 — AccessDenied는 인가 단계에서 거부됐다는 뜻이고, SignatureDoesNotMatch는 요청 서명 계산이 맞지 않아 애초에 요청자 신원 검증조차 되지 않은 서명·자격 증명 문제다.\n\n다만 AccessDenied 하나만으로 '요청이 정상적으로 인증됐다'고 단정할 수는 없다 — 서명 없는 익명 요청(예: 비공개 S3 객체에 대한 미인증 요청)도 AccessDenied를 받을 수 있기 때문이다. 원인도 identity 기반 정책(흔히 말하는 'IAM 정책') 하나로 좁힐 수 없고, 리소스 기반 정책·SCP·권한 경계·VPC 엔드포인트 정책 등 여러 계층에서 발생할 수 있다. 트러블슈팅형 문항은 이렇게 '같은 상태 코드, 다른 에러 코드' 조합을 자주 출제한다 — 상태 코드만 보고 결론 내리지 말고 에러 코드로 원인 계층을 좁혀가야 한다.",
    "choiceExplanations": [
      "정책 수정은 AccessDenied 계열에는 맞는 방향의 조치이지만, SignatureDoesNotMatch는 서명·자격 증명 문제라 정책을 고쳐도 해결되지 않는다.",
      "정답. 같은 403 상태 코드라도 에러 코드로 원인이 갈립니다 — AccessDenied는 인가 실패, SignatureDoesNotMatch는 서명·자격 증명 문제입니다. 다만 AccessDenied가 인증 성공을 보장하지도, 원인을 identity 정책 하나로 좁히지도 않습니다.",
  ...
  // source: aws-basics-spot-instance-interruption-tolerant-workload
  {
    ...
    "explanation": "스팟 인스턴스는 AWS의 유휴(사용되지 않는) EC2 용량을 온디맨드 대비 최대 90% 할인된 가격에 제공한다. 대신 AWS가 그 용량을 다시 필요로 하면 인스턴스를 회수(중단)할 수 있다 — 일반적으로 2분 전 중단 통지가 오지만, 인스턴스가 hibernate로 구성된 경우처럼 통지 없이 곧바로 시작되는 예외도 있다. 통지 방식과 무관하게 '언제든 중단될 수 있다'는 전제 자체가 핵심이므로, 배치 작업·데이터 분석·백그라운드 처리처럼 중단되어도 다시 이어서 돌리면 되는 워크로드에 적합하다.\n\n반대로 항상 떠 있어야 하는 서비스(예: 상시 운영 웹 서버)에는 스팟이 부적합하다 — 시험은 '중단을 감수할 수 있다' + '비용 최소화'라는 조합이 나오면 스팟을 정답으로 유도한다.",
    "choiceExplanations": [
