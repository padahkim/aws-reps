...
export const session: SessionData = {
  concepts: [
    ...
    // ── 13 ALB 인증 오프로드 ────────────────────────────────────────────
    {
      ...
      a: "API Gateway는 앱이 이미 받아 온 토큰을 검증한다. ALB는 토큰을 받는 대신 로그인 자체를 대신 수행한다 — 미인증 사용자를 Hosted UI로 보내고, 돌아온 인가 코드를 직접 토큰으로 교환해 UserInfo에서 클레임을 조회한 뒤, 세션 쿠키를 발급하고 인증된 요청만 클레임 헤더(x-amzn-oidc-data)와 함께 타깃 그룹으로 넘긴다.\n그래서 백엔드에서 로그인 플로우 코드를 걷어낼 수 있다. 다만 그 클레임으로 인가하려면 x-amzn-oidc-data의 서명을 검증하고 signer가 예상한 ALB ARN인지 확인하는 일은 백엔드 몫이다.",
      why: {
  ...
  mixed: [
    ...
    },
    {
      ...
      why: "ALB가 HTTPS 리스너에서 로그인 플로우를 통째로 대신하고 인증된 요청만 백엔드로 보낸다 — 로그인을 붙이는 데 백엔드 코드는 필요 없다. 백엔드가 사용자별 인가에 클레임을 쓰기 시작하면, 그때 x-amzn-oidc-data 서명 검증은 백엔드 몫이 된다.",
