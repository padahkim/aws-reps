...
export const session: SessionData = {
  concepts: [
    ...
    // ── 14 조합과 총정리 ────────────────────────────────────────────────
    {
      ...
      a: "① 앱이 User Pool로 로그인해 ID 토큰을 받고 ② 그 ID 토큰을 Identity Pool의 Logins 맵에 넣어 제출하면 임시 AWS 자격 증명이 앱으로 돌아오며(STS 호출은 Identity Pool이 뒤에서 한다) ③ 앱이 그 자격 증명으로 S3·DynamoDB를 직접 호출한다 — “인증은 CUP, 인가는 CIP”다. Access·Refresh 토큰은 이 교환에 대신 쓰지 않는다.\n다만 백엔드 API만 보호하면 되는 앱은 User Pool 하나로 끝난다(API Gateway 조합). 사용자가 AWS 리소스를 직접 만져야 할 때 비로소 Identity Pool이 붙으므로, “둘 다 써야 한다”가 항상 정답인 것은 아니다.",
    },
  ],

  /**
   * 도식 재현 — CIP 자격 증명 발급 경로 (§11 CipCredentialFlow 와 같은 ①~⑤).
   * 이 흐름은 앱을 가운데 둔 왕복이라 호출 관계 그대로는 선형 체인에 안 들어간다. 그래서
   * 체인을 “증명·자격 증명을 쥔 쪽이 바뀌는 순서”로 읽는다 — edges[i] 는 nodes[i] 에서
   * nodes[i+1] 로 무엇이 넘어가는지이고, 앱은 증명을 쥘 때와 자격 증명을 쥘 때 두 번 나온다 (#292).
   * ④는 STS → 앱으로 그렸지만 실제로는 Identity Pool 을 거쳐 온다 — 다섯째 노드의 role 에 적었다.
   * (CUP 로그인은 연합 로그인 가지가 갈라지고, ALB 흐름은 리다이렉트로 되돌아와 체인이 안 맞는다.)
   */
  diagram: {
    prompt: "앱이 S3를 직접 호출하기까지 증명과 자격 증명을 쥔 쪽이 바뀌는 순서대로 떠올려 보세요. 앱은 두 번 나옵니다.",
    nodes: [
      ...
      { role: "Identity Pool을 거쳐 자격 증명을 돌려받는 쪽", name: "앱 (웹·모바일)" },
