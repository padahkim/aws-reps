/**
 * 생성물 — 손편집 금지. `node scripts/import-drills.ts s3` 재실행으로 갱신.
 * 원본: content/drills-src/s3.json (15문항)
 */
...
export const quiz: Question[] = [
  // source: s3-presigned-url-creator-permissions
  {
    ...
    "explanation": "사전 서명 URL은 발급자가 자신의 보안 자격 증명으로 URL에 서명한 것이다. 따라서 URL을 사용하는 쪽은 별도의 AWS 자격 증명이 필요 없지만, 가능한 작업은 전적으로 URL을 만든 IAM 보안 주체가 해당 작업(예: GetObject)에 가진 권한 범위로 제한된다. 발급자에게 그 객체를 읽을 권한이 없으면 URL을 받아도 접근은 거부된다.\n\n시험에서는 '자격 증명 없는 외부 사용자에게 임시 접근'이라는 표현이 나오면 사전 서명 URL을 떠올려야 한다. 핵심 함정은 '누구의 권한이 적용되는가'다 — 받는 사람이 아니라 발급자의 권한이 적용되고, 버킷 정책을 수정하지 않고도 시간 제한 접근을 줄 수 있다는 점이 출제 포인트다.",
    "choiceExplanations": [
      ...
      "정답. 사전 서명 URL은 발급한 보안 주체의 권한을 그대로 위임하며, 발급자에게 권한이 없으면 URL도 무효입니다.",
  ...
  // source: s3-sse-kms-audit-trail-choice
  {
    ...
    "scenario": "규정 준수 팀이 한 버킷에 대해 두 가지를 요구한다 — 객체를 저장 시 암호화할 것, 그리고 어떤 키가 언제 누가 사용했는지 감사 추적(audit trail)을 남길 것. 키 사용 권한도 IAM/키 정책으로 세밀하게 통제하고 싶다. 가장 적절한 서버측 암호화 방식은?",
    "choices": [
    ...
    "explanation": "모든 S3 버킷은 기본적으로 SSE-S3로 암호화되지만, SSE-S3는 키를 S3가 전적으로 관리하므로 키 사용 권한 정책이나 CloudTrail 감사 로그를 따로 제공하지 않는다. SSE-KMS는 KMS 키를 사용하므로 키 정책·IAM으로 누가 키를 쓸 수 있는지 통제하고, KMS API 호출이 CloudTrail에 기록되어 키 사용 감사가 가능하다.\n\n시험의 분기 키워드는 '키 사용 감사', '키 권한 통제', '규정 준수'다. 이 조건이 등장하면 SSE-S3가 아니라 SSE-KMS가 답이 된다. 단, SSE-KMS는 KMS 요청 쿼터의 영향을 받으므로 대량 요청 시 S3 버킷 키로 비용·요청을 줄이는 후속 고려가 따라온다.",
    "choiceExplanations": [
      "SSE-S3는 키 관리를 S3에 위임하므로 키 사용 권한 통제나 CloudTrail 감사 추적을 제공하지 않는다 — 감사 요구를 충족하지 못한다.",
      "정답. 키 사용 권한의 세밀한 통제와 감사 추적이 필요하면 답은 SSE-KMS입니다.",
  ...
  // source: s3-multipart-upload-large-file-resilience
  {
    ...
    "explanation": "멀티파트 업로드는 객체 하나를 독립된 여러 파트로 나눠 병렬·임의 순서로 올린다. 파트가 실패하면 그 파트만 다시 올리면 되므로 전체 재업로드를 피할 수 있고, 병렬 전송으로 처리량도 높아진다. AWS는 100MB 이상 객체에 멀티파트 업로드를 권장한다.\n\n출제 포인트는 '대용량 + 불안정한 네트워크 + 재시도 비용'이라는 키워드 묶음이다. 이 조합에서는 멀티파트 업로드가 정답이다. 완료되지 않은 멀티파트 업로드 파트는 저장 비용을 발생시키므로, 수명 주기 규칙으로 미완료 업로드를 정리하는 후속 관리도 함께 알아두면 좋다.",
    "choiceExplanations": [
  ...
  // source: s3-cors-browser-preflight-blocked
  {
    ...
    "explanation": "CORS는 한 도메인에서 로드된 웹 애플리케이션이 다른 도메인의 S3 리소스에 접근하도록 허용하는 메커니즘이다. 브라우저가 보내는 프리플라이트 요청을 받으면 S3는 버킷의 CORS 구성에서 요청의 출처·HTTP 메서드·헤더가 매칭되는 규칙을 평가한다. 따라서 app.example.com을 AllowedOrigins에, 사용하는 메서드를 AllowedMethods에 넣어야 차단이 풀린다.\n\n출제 포인트는 'CORS 오류는 권한 문제가 아니라 출처 허용 구성의 문제'라는 구분이다. IAM·버킷 정책이 충분해도 CORS 구성이 없으면 브라우저가 응답을 막는다. CORS는 권한을 대체하지 않으며, 기존 ACL·정책은 그대로 함께 적용된다는 점도 기억한다.",
    "choiceExplanations": [
  ...
  // source: s3-presigned-url-role-credential-expiry
  {
    ...
    "explanation": "사전 서명 URL의 유효 기간은 URL을 만든 자격 증명의 유형에 묶인다. IAM 사용자의 장기 키(SigV4)로 만들면 최대 7일까지 가능하지만, IAM 역할·STS 같은 임시 자격 증명으로 만들면 그 자격 증명이 만료·취소되는 순간 URL도 만료된다 — 더 긴 만료 시간을 지정했더라도 마찬가지다. EC2 인스턴스의 역할 자격 증명은 보통 수 시간 단위로 갱신되므로 7일을 채우지 못한다.\n\n출제 포인트는 '발급에 쓴 자격 증명 종류가 실제 만료를 좌우한다'는 점이다. 정말 최대 7일이 필요하면 IAM 사용자 장기 키로 서명해야 한다. 다만 키 노출 위험 때문에 실무에서는 더 짧은 만료와 임시 자격 증명을 선호한다는 트레이드오프도 함께 이해한다.",
    "choiceExplanations": [
      "버그가 아니라 임시 자격 증명의 정상 동작이다. 자격 증명이 만료되면 그 자격 증명으로 서명한 URL도 함께 만료된다.",
      "CLI/SDK로는 최대 7일까지 지정할 수 있다(콘솔은 최대 12시간). '항상 1시간'은 사실이 아니다.",
      "정답. 사전 서명 URL은 서명에 쓴 자격 증명이 만료되면 함께 만료되므로, 임시 역할 자격 증명으로 만들면 지정한 7일보다 짧게 끝납니다.",
