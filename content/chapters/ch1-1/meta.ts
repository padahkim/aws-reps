...
export const chapterMeta: ChapterMeta = {
  ...
  objectives: [
    ...
    "버전 관리·복제의 전제와 한계(기존 객체 미복제·체이닝 불가)를 근거로 복구 시나리오를 세운다",
    "prefix당 3,500/5,500 요청, 객체 최대 50TB·단일 PUT 5GB, Presigned URL 기본 3600초를 바로 떠올린다",
  ],
  // 18섹션을 6묶음으로. 경계는 "객체에 무슨 일이 일어나는가"의 단계 기준 — 담고 열기(01~03) →
  // 시간이 지나며 생기는 일(04~07) → 붙는 장치(08~10) → 암호화(11~12) → 켜는 설정(13~15) →
  // 남에게 열어 주는 법(16~18). 암호화는 §11이 이 챕터 최다 출제라 이웃과 섞지 않고 따로 뒀다.
  parts: [
    ...
    { title: "연동 · 삭제 방지 · 감사 설정", from: "13", to: "15" },
    { title: "임시 접근과 대규모 접근 관리", from: "16", to: "18" },
  ],
};
