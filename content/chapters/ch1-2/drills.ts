 ...
 * 원본: content/drills-src/lambda.json (15문항)
 */
...
export const quiz: Question[] = [
  ...
  // source: lambda-throttling-429-mitigation
  {
    ...
    "choices": [
      ...
      "해당 함수에 예약 동시성(reserved concurrency)을 설정해 다른 함수가 동시성 풀을 소진해도 처리 용량을 보장한다."
    ],
    "answer": [
      1,
      3
    ],
    "explanation": "스로틀링은 동시 실행 수가 사용 가능한 동시성을 초과할 때 발생한다. 동기 호출에서는 429가 호출자에게 그대로 반환되므로, 클라이언트의 지수 백오프+지터 재시도가 순간적인 스파이크를 시간축으로 분산시키는 1차 방어가 된다.\n\n계정의 동시성은 리전 내 함수들이 공유하는 풀이다. 예약 동시성을 설정하면 그 수치만큼 이 함수 전용 용량이 분리되어, 다른 함수의 트래픽 폭증이 이 함수의 스로틀링으로 전이되지 않는다. 근본적으로 용량이 부족하다면 계정 동시성 한도 상향 요청까지 검토한다.",
    "choiceExplanations": [
      "타임아웃은 실행 중인 함수의 최대 실행 시간 설정이다. 스로틀링은 실행 환경을 얻지 못해 시작조차 못 한 상태라 타임아웃과 무관하다.",
      "정답. 재시도(지수 백오프+지터)로 순간 스파이크를 흡수하고, 예약 동시성으로 이 함수 몫의 처리 용량을 확보합니다.",
  ...
  // source: lambda-sqs-batch-window-small-invocations
  {
    ...
    "choices": [
      "함수의 예약 동시성을 늘려 한 번에 더 많은 메시지를 받게 한다.",
      "큐의 가시성 타임아웃(visibility timeout)을 늘려 메시지가 더 오래 쌓이게 한다.",
    ...
    "choiceExplanations": [
      "예약 동시성은 이 함수가 쓸 수 있는 동시 실행 용량을 정할 뿐, 한 배치에 담기는 메시지 수와는 무관하다.",
      "가시성 타임아웃은 처리 중 메시지가 다른 소비자에게 숨겨지는 시간이다. 한 배치에 모이는 메시지 수를 늘리는 설정이 아니다.",
  ...
  // source: lambda-envvar-customer-managed-kms-key
  {
    ...
    "choices": [
      ...
      "환경 변수 암호화에 고객 관리형 KMS 키(customer managed key)를 지정하고, 그 키에 kms:Decrypt 권한이 있는 사용자만 환경 변수를 보거나 관리할 수 있게 한다.",
    ...
    "explanation": "Lambda는 환경 변수를 항상 KMS 키로 저장 시 암호화하며, 기본값은 AWS 관리형 키다. 기본 키를 쓰면 사용자나 실행 역할에 별도 KMS 권한이 필요 없어 누구나(함수 관리 권한만 있으면) 환경 변수를 볼 수 있다.\n\n고객 관리형 키를 지정하면 그 키에 kms:Decrypt 권한이 있는 사용자만 환경 변수를 열람·관리할 수 있다. 권한이 없는 사용자는 함수는 관리해도 환경 변수 값은 보지 못한다. 키 교체 주기 등도 직접 통제할 수 있어 '열람 통제 + 키 교체 통제'라는 두 요구에 정확히 맞는다. 자동 교체가 필요한 DB 자격 증명이라면 Secrets Manager가 답이지만, 여기서는 그 조건이 없다.",
    "choiceExplanations": [
  ...
  // source: lambda-ephemeral-storage-tmp-sizing
  {
    ...
    "choices": [
      ...
      "함수의 임시 스토리지(ephemeral storage, /tmp) 크기를 기본 512MB에서 필요한 만큼(최대 10,240MB) 올린다."
    ],
    "answer": [
      3
    ],
    "explanation": "Lambda는 /tmp 디렉터리에 실행 환경별 임시 스토리지를 제공하며 기본 크기는 512MB다. 이 임시 스토리지 설정은 512MB부터 10,240MB까지 1MB 단위로 따로 구성할 수 있다. 큰 모델 다운로드, ETL 중간 산출물, 미디어·PDF 처리처럼 일시적으로 큰 디스크가 필요한 워크로드의 표준 해법이다.\n\n임시 스토리지 크기는 메모리(configuration-memory) 설정과 별개의 값이다. 메모리를 올린다고 /tmp가 따라 늘어나지 않는다 — '메모리=CPU·RAM, ephemeral storage=/tmp 디스크'로 구분하는 것이 출제 포인트다.",
    "choiceExplanations": [
      "메모리 설정은 RAM과 CPU에 영향을 줄 뿐, /tmp 디스크 용량과는 별개의 설정이다. 메모리를 올려도 /tmp는 그대로다.",
      "임시 스토리지를 늘리면 모델을 통째로 /tmp에 둘 수 있으므로, 스트리밍 재작성이 '유일한 방법'이라는 단정은 틀렸다.",
      "타임아웃은 실행 시간 한도일 뿐 디스크 용량과 무관하다.",
      "정답. /tmp 임시 스토리지는 512MB~10,240MB로 따로 설정하는 값이며, 메모리 설정과는 별개입니다."
    ],
