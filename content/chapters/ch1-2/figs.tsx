...
import { C, FigSwitch, MONO } from "../ui";
...
export function VpcReachabilityBoard() {
  ...
  const rows: { target: string; ok: boolean; why: string }[] = [
    ...
    {
      // 이 보드의 축은 네트워크 경로다. "항상 ✔"로 단언하면 §09 과 같은 챕터 셀프퀴즈가
      // 가르치는 전제(로그가 안 남으면 실행 역할 권한부터 의심)와 정면으로 어긋난다
      // — 옆 RDS 행이 IAM·SG 전제를 밝히는 것과도 비대칭이었다 (PR #151 Codex 지적).
      target: "CloudWatch Logs",
      ok: true,
      why: "네트워크는 NAT·엔드포인트 없이 열린다 (실행 역할의 로그 쓰기 권한은 별도 전제 — §09)",
    },
  ];
