 ...
 *     행 선행 들여쓰기를 잘라먹는다.
 *
 * 섹션 분량 (#186 — 숫자 상한은 폐지했다):
 *   • **분량 게이트는 없다.** 옛 규약(#148 채택안 2 → #162)은 섹션 본문 ~3KB 상한을 두고
 *     검증기가 초과를 warn 했으나, #186 조사에서 근거가 없어 폐지했다. 요지 3가지 —
 *     ① 3KB 는 ch1-2 한 챕터의 관측 최대값을 반올림한 값이고 그 위에 얹을 근거가 없다.
 *     ② 섹션당 바이트는 콘텐츠 양이 아니라 "챕터를 얼마나 잘게 썰었나"를 잰다 — 챕터 총량은
 *     1.9배 차이인데 섹션당 소요는 3.1배 차이라, 18~20섹션 챕터는 전부 통과하고 4섹션
 *     챕터는 전부 위반했다(#148 자신의 진단인 "동형 반복"과 신호가 정반대).
 *     ③ 실제 소요와도 어긋난다 — 본문 바이트만 세고 셀프퀴즈·인출 카드(섹션 시간의 30~50%)를
 *     안 세서, 5분 넘는 섹션 20개 중 8개를 놓쳤다.
 *   • **분량은 소요 표시로 본다** — lib/reading-time.ts 가 매 빌드 산출해 홈·목차·파트에
 *     "약 N분"으로 띄운다(#161·#173). 임계값 없이 사람이 보고 판단하는 쪽이 정본이다.
 *     새 숫자 게이트를 만들지 않는다 — 그게 폐지한 규약이 저지른 실수(관측값 승격)다.
 *   • <Fold>(ui.tsx)는 남는다 — **게이트가 시키는 처방이 아니라 작성자가 쓰는 도구**로.
 *     시험 비직결 상세(범위 밖 서비스, 구버전 자료 정정 등)를 접으면 접힌 제목
 *     ("더 알아보기 — <주제>")만 보여 건너뛰어도 학습 경로가 끊기지 않는다.
 *     단 인출 카드·셀프퀴즈가 묻는 재료는 접지 않는다 — 접힌 채 지나가면 #164 가 메운
 *     결손("본문에 재료가 없는 물음")이 같은 모양으로 되살아난다.
 *
