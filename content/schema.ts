 ...
 *   • parts (청킹 — Miller / 분할 원리 — Mayer) — 섹션을 4~6개 묶음으로 나눠 중간 완결감을
 *     준다. 목차가 파트로 그룹핑되고 파트별 진도가 붙으며, 섹션 페이지 상단에 파트 컨텍스트가
 *     뜬다. from/to 는 SectionMeta.num — 검증기가 전 섹션을 빠짐·중복 없이 덮는지 강제한다
 *     (덮지 못하는 분할은 목차에서 섹션이 조용히 사라지는 결과가 되므로 빌드를 막는다).
