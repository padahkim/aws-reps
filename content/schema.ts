 ...
 *     행 선행 들여쓰기를 잘라먹는다.
 *
 * 섹션 분량 예산 (#148 채택안 2 → #162):
 *   • 섹션 본문(sections/NN.mdx)의 예산 상한은 ~3KB (소스 UTF-8 바이트. #148 실측:
 *     ch1-2 평균 1.6KB·최대 3.2KB 를 반올림한 값 — 표본은 그 한 챕터다).
 *     ⚠ "이미 대체로 지켜지는 값"은 아니다: 규약 착지 시점(9c40511)에 이미 ch0-1(최대
 *     9.5KB)·ch0-2(최대 5.6KB)의 12개 섹션이 초과였다. 상한의 근거 재검토는 #186.
 *   • 초과분은 시험 비직결 상세부터 <Fold>(ui.tsx)로 접는다 — 본문 = 시험 직결, 심화 = Fold.
 *     접힌 제목("더 알아보기 — <주제>")만 보이므로 심화는 건너뛰어도 학습 경로가 끊기지 않는다.
 *   • 검증기(validate-content)는 <Fold …>…</Fold> 블록을 제외한 바이트로 재고, 상한 초과는
 *     warn 만 한다(빌드 실패 아님) — 분량은 품질 신호지 계약 위반이 아니다.
 *   • 기존 챕터 전면 재태깅은 하지 않는다(#148 기각 — 위 실측이 근거) — 상한 초과 섹션만
 *     기회 적용하고, 신규 챕터는 변환 시 적용한다(#29 템플릿 몫).
 *
