...
# CI 게이트 (#137) — Codex 게이트와 같은 위상의 세 번째 전제. --watch 는 pending 을 완료까지
# 기다리고, --fail-fast 는 첫 실패에서 바로 나온다. 실패하면 머지하지 않고 실패한 체크
# 이름·사유를 사용자에게 보고한다 (우회는 위 "우회는 사용자의 명시 지시로만" 규칙과 동일)
gh pr checks "$PR" --watch --fail-fast || exit 1
...
- **CI 게이트가 왜 여기 있나 (#137 — PR #134 사고)**: CI `verify` 잡이 실패했는데 B-3이 리뷰·승인만 보고 머지해 develop이 빨간불로 남았다(사후 수습 c31eb08). `gh pr checks --watch`는 pending을 완료까지 기다리므로 push 직후의 일시 pending을 실패로 오판하지 않는다. 로컬 쪽 같은 구멍은 `npm run validate`가 `gen-arch-facts.ts --check`를 연쇄 실행해 막는다 (같은 이슈).
