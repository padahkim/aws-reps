...
**보고 직전에 `ASK_HEAD`를 포착해 보고에 함께 적는다** — 승인은 "제시한 head"에 묶이므로(B-3), 제시가 없는 승인은 B-3에서 무효가 되어 2차 승인을 강제한다:

```bash
ASK_HEAD=$(gh api "$API"/pulls/"$PR" --jq .head.sha)
[ -n "$ASK_HEAD" ] || exit 1   # 빈 값을 승인 대상으로 제시하지 않는다
echo "머지 승인 요청 — head $ASK_HEAD"   # 이 리터럴 값을 보고문에 적고, B-3까지 그대로 들고 간다
```

PR URL·"PR 대기"·**제시한 head SHA**를 사용자에게 보고한다. **여기까지 하면 이 세션의 착지 의무는 충족** — 머지는 승인 후에 한다. 승인을 기다리는 동안 손을 놓지 말고 **B-2(자동 리뷰 확보)를 이어서 진행한다** — 리뷰는 승인과 무관하게 확보해야 하고, 먼저 돌려두면 승인이 떨어졌을 때 바로 머지할 수 있다.

- **왜 B-3이 아니라 여기인가** (#133 P2, 원 지적은 PR #129 라운드 2). 포착이 B-3에 있으면, 정상 흐름에서 **실행이 B-3에 닿기 전에 B-1이 승인을 기다린다** — 그래서 첫 승인은 언제나 "head 제시 없는 승인"이 되고, B-3의 규칙이 그것을 무효화해 불필요한 2차 승인을 부른다. 제시를 B-1로 옮기면 첫 승인을 그대로 재사용할 수 있다.
- **B-2에서 지적을 반영해 push했다면 `ASK_HEAD`는 낡는다** — head가 바뀌었으므로 **새 head를 제시해 승인을 다시 받고** `ASK_HEAD`를 그 값으로 갱신한다. 이건 규칙의 예외가 아니라 규칙 그대로다(사용자가 본 적 없는 코드는 승인된 적이 없다).
...
TRIG=$(gh api -X POST "$API"/issues/"$PR"/comments -f body="@codex review" --jq .id)
[ -n "$TRIG" ] || exit 1               # 트리거 코멘트 id — (d)가 이 코멘트의 리액션도 본다 (#132)
echo "트리거 코멘트 id=$TRIG"
```
...
#     두 자리를 **합쳐서** 본다: PR 본문과 트리거 코멘트. 어디에 붙을지는 실행마다 다르다 (#132)
#     시각 필터를 걸지 않는다 — 자동 발동의 본문 👀는 대개 $SINCE 직전이라 걸러져 버린다.
#     대신 created_at 을 함께 찍어 세션이 직접 판단한다 (어차피 판정으로는 쓰지 않는다)
{ gh api --paginate "$API"/issues/"$PR"/reactions
  gh api --paginate "$API"/issues/comments/"$TRIG"/reactions; } | jq -s 'add' \
  | jq --argjson cx "$CX" '[.[] | select(.user.login|IN($cx[])) | {content, at:.created_at}]'
```
...
- **리액션이 어디에 붙는지는 실행마다 다르다 — 그래서 (d)는 두 자리를 합쳐 본다** (#132). 2026-07-26 PR #129에서는 자동 발동이 **PR 본문**에(`12:11:52`), 수동 트리거가 **그 트리거 코멘트**에(`12:12:07`) 👀을 달았다. 반면 2026-07-27 PR #167에서는 수동 트리거인데도 👀이 **PR 본문**에 붙었다(트리거 코멘트에는 없었다). 한 자리만 보면 접수된 실행을 "👀 없음"으로 읽어 불필요한 재트리거를 한다.
- **👀은 끝나면 회수된다 — 그래서 "👀 없음"은 "접수 안 됨"의 증거가 아니다** (2026-07-27 PR #167 실측). 라운드 1에서 본문 👀이 보이다가 지적 리뷰가 도착하는 순간 사라졌고, 라운드 2에서는 클린 코멘트와 **같은 초**에 👍(`+1`)로 바뀌었다(`21:11:37` ↔ 클린 코멘트 `21:11:38`). 즉 👀의 부재는 "아직 접수 전"과 "이미 끝남"을 구분하지 못한다 — **판정을 먼저 확인하고, 판정이 없을 때만 👀을 본다** (아래 3번 순서). 완료 시 붙는 👍도 판정이 아니다(위 "맨몸 👍" 항목) — head를 담지 않으므로 그대로 클린으로 쓰지 않는다.
...
**순서가 있다 — 판정을 먼저 보고, 없을 때만 👀을 본다** (#132). 👀은 완료 시 회수되므로, 순서를 뒤집으면 이미 끝난 실행을 "접수 안 됨"으로 읽는다.

- **지금 head를 본 판정이 있으면** 그것으로 끝이다 — 리액션은 볼 필요가 없다.
- **판정이 없고 👀가 (본문이든 트리거 코멘트든) 붙어 있으면 접수된 것** — 상한까지 기다린다.
- **판정도 👀도 없이 5분이 지나면** 트리거가 유실됐을 가능성이 크다 — 10분을 채우지 말고 **한 번 재트리거**하고 `$SINCE`·`$TRIG`를 새로 잡는다.
...
- **클린** — **(b)의 합산이 0건인 것이 두 경로 공통의 전제다.** 그 위에서 ① 지금 head를 본 리뷰가 **전부** 인라인 0건이고 요약에도 지적이 없거나, ② 지금 head를 적은 클린 코멘트가 있으면 클린이다. → **승인이 지금 head에 대한 것이면** 다시 묻지 않고 B-3으로 진행한다.
  - **"또는"이 아니라 "그리고"다** (#133 P1, 원 지적은 PR #129 라운드 2). 자동 발동과 수동 트리거가 같은 head를 보면 **한쪽은 인라인 지적을, 다른 쪽은 클린 요약 코멘트를 낼 수 있다.** 옛 문장은 클린 코멘트 가지를 독립 조건으로 둬서, `$RIDS` 합산이 0이 아닌데도 클린이 성립했다 — 바로 위 "클린은 `$RIDS`의 **모든** 리뷰가 0건일 때만"과 정면으로 모순이고, 첫 실행의 지적을 남긴 채 머지된다. 클린 코멘트는 **지금 head의 인라인 지적이 0건일 때만** 판정으로 성립한다. 보고에 "Codex 리뷰 지적 0건"을 적는다. 이 게이트의 목적은 리뷰를 확보하는 것이지 승인을 두 번 받는 것이 아니다.
...
**전제 셋이 모두 참이어야 머지한다** — (i) B-2의 리뷰 확보가 **지금 head에서** 끝났다, (ii) 사용자 승인이 **지금 head에 대한 것**이다, (iii) PR의 CI 체크가 **전부 통과**했다 (#137 — 아래 CI 게이트). (i)이 아니면 "리뷰 대기 중", (ii)가 아니면 "재승인 필요", (iii)이 아니면 "CI 실패 — 체크 이름과 사유"를 보고하고 여기서 멈춘다.

**우회는 사용자의 명시 지시로만** (2026-07-26 결정, #124). B-2가 타임아웃(10분)이나 라운드 상한(2회)으로 끝났을 때, 사용자가 "리뷰 없이 머지해"·"남은 지적 거부하고 머지해"라고 **명시**하면 그때만 진행한다 — Codex 장애가 착지를 영구히 막지 않게 하는 탈출구다. 그 경우 보고에 **"리뷰 미확보 머지"** 또는 **"미반영 지적 n건 남긴 채 머지"**를 반드시 적는다. **CI 게이트(#137)도 같은 규칙이다** — CI 실패로 멈췄을 때 사용자가 "CI 실패 무시하고 머지해"라고 명시하면 그때만 게이트를 건너뛰고, 보고에 **"CI 실패 머지"**와 실패한 체크 이름을 적는다. 승인 독려("쭉 해줘")는 우회 지시가 아니다. 우회해도 **아래 head 대조는 건너뛰지 않는다** — 우회하는 건 리뷰 확보이지 "사용자가 승인한 코드가 머지된다"는 보장이 아니다.

**승인은 "제시한 head"에 묶는다** (2026-07-26, #127 — 현행은 "승인은 head에 묶인다"를 문장으로만 선언해서, 승인 뒤 다른 세션·협업자·자동화가 브랜치를 갱신하면 세션이 알아채지 못한 채 **리포의 유일한 비가역 관문**을 통과했다). 순서가 핵심이다 — **승인을 요청할 때 head SHA를 함께 제시하고**(그 자리는 **B-1**이다), **승인이 오면 그 제시한 값을 그대로 옮겨 적는다**:

```bash
# ASK_HEAD 는 **B-1에서 이미 포착해 보고에 적은 리터럴 값**이다 (#133 P2).
# 여기서 다시 조회하지 않는다 — 조회하면 "제시한 값"이 아니라 "지금 값"이 되어 대조가 무의미해진다.
# (B-2에서 push가 있었으면 B-1의 재승인 규칙대로 새 head를 제시해 다시 받았을 것이고,
#  ASK_HEAD 는 그 값으로 갱신돼 있다.)
APPROVED_HEAD=<B-1에서 제시한 SHA>   # 승인 도착 후, 제시했던 값을 그대로 옮겨 적는다
[ -n "$APPROVED_HEAD" ] || exit 1     # 제시한 값이 없으면 승인이 없는 것이다 — 다시 받는다
```
...
# CI 게이트 (#137) — Codex 게이트와 같은 위상의 세 번째 전제. REST 로 본다 (`gh pr checks` 는
...
[ "$(echo "$PENDS" | jq length)" -eq 0 ] || { echo "CI 진행 중: $PENDS"; exit 1; } # 실패가 아니다 — 완료까지 재조회해 이 블록을 다시 돈다 (상한 10분, 넘기면 보고)
...
- **CI 게이트가 왜 여기 있나 (#137 — PR #134 사고)**: CI `verify` 잡이 실패했는데 B-3이 리뷰·승인만 보고 머지해 develop이 빨간불로 남았다(사후 수습 c31eb08). pending은 실패와 구분해 재조회한다 — push 직후의 일시 pending을 실패로 오판하지 않기 위해서다. 로컬 쪽 같은 구멍은 `npm run validate`가 `gen-arch-facts.ts --check`를 연쇄 실행해 막는다 (같은 이슈).
...
develop 최신 해시, 경로(즉시/PR)와 머지 방식(ff/머지커밋), push 결과, **CI 체크 결과**(통과 / 실패 체크 이름 — PR 착지만 해당), **Codex 리뷰 결과**(지적 0건 / 반영 n건 / 미도착 — **판정받은 head SHA와 머지한 head SHA를 함께** 적는다. 둘이 같다는 게 게이트가 실제로 작동했다는 유일한 증거다), 보드 갱신 결과(또는 "보드 미갱신"), 이슈 체크박스 갱신 결과(미달성 항목 포함, 또는 "체크박스 미갱신"), 잔재 정리 결과(삭제한 브랜치 수, **훑은 워크트리 경로와 건수**, 제거한 워크트리, 건너뛴 워크트리와 사유 — 사용 중 / 판별 불가 / 최근 활동 / 더러움 / 미착지)를 사용자에게 보고한다. PR 착지 대기 상태면 PR URL과 사유를 보고한다.
