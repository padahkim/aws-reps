...
if [ -n "$CWDS" ]; then
  GRACE=60                                                    # 분(=1시간). 직접 판별이 되므로 mtime은 백스톱만
else
  GRACE=720                                                   # 분(=12시간). 직접 판별 불가 → 옛 유예로 폴백
  echo "  주의: 프로세스 cwd 판별 불가(lsof 없음/차단) — mtime 12시간 유예로 폴백"
fi
...
- 그래도 **판별 불능일 때는 지우지 않는다** — `lsof`가 없거나 막혀 `$CWDS`가 비면 `GRACE`를 옛 12시간으로 되돌린다. 비용이 비대칭이기 때문이다: 잔재를 못 지우면 다음 세션이 지우면 그만이지만, 살아 있는 워크트리를 지우면 병렬 세션이 그 자리에서 죽는다.
...
- 건너뛴 워크트리가 있으면 **사유(사용 중 / 최근 활동 / 더러움 / 미착지)와 함께** 보고에 명시한다 — 사용자가 병렬 세션 여부를 판단한다.
