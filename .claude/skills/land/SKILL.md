...
# (b) 지금 head를 본 리뷰의 인라인 지적만 — 그 리뷰 id에 묶어서 센다
RID=$(gh api --paginate "$API"/pulls/"$PR"/reviews | jq -s 'add' \
  | jq --argjson cx "$CX" --arg head "$HEAD" \
    '[.[] | select((.user.login|IN($cx[])) and .commit_id == $head)] | last | .id // empty')
gh api --paginate "$API"/pulls/"$PR"/comments | jq -s 'add' \
  | jq --argjson rid "${RID:-null}" '[.[] | select(.pull_request_review_id == $rid)]
    | {n: length, items: [.[] | {id, loc:"\(.path):\(.line)", body}]}'
...
# (d) PR 본문 리액션 — 이번 라운드 Codex 것만 (리액션에도 created_at이 있다)
...
# (e) PR 생성 후 push가 있었나 — (d)의 맨몸 👍를 판정으로 쓸 수 있는지 가른다
gh pr view "$PR" --json createdAt,commits \
  --jq '{pushedAfterOpen: (.commits[-1].committedDate > .createdAt)}'
```
...
- **`RID`가 비면 (b)의 `n`은 0이 되지만 그건 클린이 아니다** — "지금 head를 본 리뷰가 아직 없다"는 뜻이다. 0을 클린으로 읽는 건 이 게이트가 막으려는 사고 그 자체다. 클린은 `RID`가 있고 그 리뷰의 `n == 0`일 때, 또는 아래 두 경로(클린 코멘트·인정 조건을 갖춘 👍)일 때만이다.
...
- **맨몸 👍(본문 없는 리액션)은 head를 담지 못한다** — `pushedAfterOpen == false`일 때만, 즉 PR 생성 후 head가 한 번도 안 바뀌었을 때만 판정으로 인정한다. push가 있었으면 묵은 실행의 👍일 수 있으므로 판정으로 쓰지 않고 head를 적은 판정을 계속 기다린다 (그래도 안 오면 3)의 상한 처리). PR #122 실측: 자동 발동 클린은 리액션만 오고 코멘트가 없다. **세션 기억에 의존하지 말 것** — 착지가 다음 세션으로 넘어가면 그 기억이 없고, `pushedAfterOpen`은 리포에서 다시 읽을 수 있다.
...
- **리액션은 두 종류를 구분한다.** `+1`(👍)은 "지적 없음"이라는 **판정**, `eyes`(👀)는 "트리거를 접수하고 작업 중"이라는 **진행 신호**다. 👀을 클린으로 오인하면 리뷰가 끝나기도 전에 머지한다 — 이 게이트가 막으려는 바로 그 사고다. 👀만 있으면 계속 기다린다.
...
- **판정 없음** (지금 head를 본 리뷰·클린 코멘트가 없고, 인정 조건을 갖춘 맨몸 👍도 없음) → 아직 판정이 아니다. 계속 기다린다. 이전 head를 본 판정이 새로 도착했더라도 **무시하고**, 무시했다는 사실을 보고에 적는다 (묵은 판정을 클린으로 쓰지 않는다).
- **클린** (지금 head를 본 리뷰의 인라인 0건 **그리고** 요약에 지적 없음 — 지금 head를 적은 클린 코멘트나 인정 조건을 갖춘 👍만 온 경우 포함) → **승인이 지금 head에 대한 것이면** 다시 묻지 않고 B-3으로 진행한다. 보고에 "Codex 리뷰 지적 0건"을 적는다. 이 게이트의 목적은 리뷰를 확보하는 것이지 승인을 두 번 받는 것이 아니다.
...
**승인을 받은 순간 그 head를 기록한다** (2026-07-26, #127 — 현행은 "승인은 head에 묶인다"를 문장으로만 선언해서, 승인 뒤 다른 세션·협업자·자동화가 브랜치를 갱신하면 세션이 알아채지 못한 채 **리포의 유일한 비가역 관문**을 통과했다):

```bash
APPROVED_HEAD=$(gh pr view "$PR" --json headRefOid --jq .headRefOid)   # 승인을 받은 그 자리에서 즉시
```
