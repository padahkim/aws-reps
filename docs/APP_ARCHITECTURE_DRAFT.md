...
> **현행 구현물은 이 문서가 아니다** — as-built 아키텍처·건강 진단은 [`docs/ARCHITECTURE.md`](ARCHITECTURE.md)를 본다. 이 초안이 상정한 구조(어댑터 계층·`<Quiz>` context·오답노트·Leitner 저장소)가 실제로 무엇으로 접혔거나 미구현인지는 그 문서 §5-대조에 정리돼 있다. 이 문서는 **설계 의도의 기록**으로 남긴다.
>
