...
- 규약 v1 확정일: (미확정) / 낙점된 표준 표현 형식: (미확정) / v0 대비 변경 요약: (미확정)
...
3. **표준화**: 생존 jsx에 `meta`·`quiz` export 부여, 문항을 공용 컴포넌트 데이터로 이전, 본문을 낙점 형식 기준으로 정돈. **내용 수정 금지**(내용 수정은 평가 리포트의 수정 지시를 따르는 별도 커밋). 원본은 `legacy/content-jsx/` 또는 브랜치 보존. 결손 성분은 축1 리포트의 보충 생성 목록대로 별도 생성
4. **앱**: 핸드오프(PLAYBOOK §5 델타 반영) → 앱 코드 진단 → 셸 구현(로더 → 네비+진도 → 공용 Quiz) → 표준화된 /content 실데이터 검증
5. **신규 모드 전환** → 챕터 생성 프롬프트를 "규약 v1 준수 jsx 모듈 생성"으로 갱신 → 갭 챕터 생성(신규 파이프라인 통과 필수)
