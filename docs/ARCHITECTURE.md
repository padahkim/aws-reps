...
| `"dva.progress.v1"` | `lib/progress/store.ts` | 학습 진도 — 문항별 채점 사실 `{ [전역 문항 키]: { attempts, correct, lastResult, lastAt } }` (#66) |

- **전역 문항 키**는 `lib/progress/keys.ts`가 `` `${chapterId}:${questionId}` ``로 합성한다. 저장 데이터가 콘텐츠 규약에 거는 **유일한 하드 의존**이라(설계 §4-2), 검증기가 두 성분의 유일성과 `:` 미포함을 강제한다.
- **점수는 저장하지 않는다**. 목차의 "8/11" 배지는 `scope === "final"` 문항의 `lastResult`를 런타임 집계해서 낸다 — 파생 가능한 값을 저장하지 않는다는 설계 원칙(§4-1) 때문이고, 덕분에 재응시가 자동 반영된다.
- **구조 버전**: 키 접미 `.v1`이 메이저, 내부 `v` 필드가 마이너다. 로드 시 read-repair(누락 필드 기본값 주입, 미지 필드 보존)를 하고, **모르는 상위 `v`는 낮추지 않는다**. 쓰기는 원본 위에 고친 값을 얹어, 못 고친 항목이 무관한 채점 때문에 지워지지 않게 한다.
