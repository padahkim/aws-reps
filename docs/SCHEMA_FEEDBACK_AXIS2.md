...
| sam_guide.jsx | `TrafficShift` — 모드(Canary/Linear/AllAtOnce) 선택 버튼 + 상태 기계형 단계 재생(사전훅→전환→사후훅→완료, 오류 토글 시 롤백 분기까지 애니메이션) | 기존 `interactiveDemo`/`interactive:{type,params}` 제안(aws-elastic-beanstalk-guide 등)과 동일 범주 — "정상/실패 분기까지 시뮬레이션하는 상태 기계형" 변형 사례로 추가, 채택 우선순위 신호 | TrafficShift 컴포넌트 전체(라인 620~ 부근) |
| sam_guide.jsx | `SamTransform` — 짧은 SAM 템플릿(클릭 전) → 버튼 클릭 시 다수의 CloudFormation 리소스가 순차 페이드인(클릭 후), "축약 문법이 무엇으로 전개되는지"를 직접 시연 | aws-cdk-dva-guide.jsx 제안(파이프라인 다이어그램 표준화)과 병합 검토 — "before/after 토글형 전개 다이어그램" 하위유형으로 `diagram: {type:"expand", before, after[]}` 검토 | SamTransform 컴포넌트(라인 454~615) |
