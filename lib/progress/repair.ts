/**
 * 학습 진도 저장소 `dva.progress.v1` 의 **순수 규칙** — 저장된 값을 읽어 다른 값으로 바꾸는
 * 모든 판단이 여기 있다. 브라우저 접점(localStorage·React)은 `records.ts` 쪽이다.
 *
 * 왜 갈라 두나 (#214): 이 파일의 결함은 타입이 맞고 화면도 멀쩡한데 **결과값만 틀린** 모양으로
 * 나온다(PR #202 에서 반복 발생). 그런 건 실행해 봐야 잡히는데, `"use client"` + react import
 * 를 달고 있으면 node 가 직접 부를 수 없어 CI 가 한 줄도 실행하지 못한다. 여기엔 import 가
 * 하나도 없으므로 `repair.test.ts` 가 그대로 불러 회귀를 잠근다.
 *
 ...
 * 경로가 없다** — 쓰는 곳이 applyAttempt 하나뿐이고 항상 앞뒤 맞는 값만 쓴다.
...
/**
 * 채점 사실 하나를 얹은 새 저장소를 낸다 — 이 키의 **유일한 쓰기 규칙**이다.
 * 시각을 인자로 받는 이유: 규칙에서 시계를 걷어내야 기대값을 적을 수 있다 (호출부가
 * `new Date().toISOString()` 을 넘긴다).
 *
 ...
 *
 * 넘어오는 `data` 는 `repair` 를 거친 값이다 — 말이 안 되는 기록은 이미 버려졌고, 호출부의
 * 저장이 그 삭제를 저장소에 반영한다 (버리는 단위는 그 문항 하나다 — repairQuestion 참조).
 */
  ...
  return {
    ...
    questions: {
      ...
      [gk]: {
        ...
        // (firstResult 가 생기기 전에 저장된 기록)는 **모르는 채로 둔다** — 나중 시도를 첫
        // 시도로 적으면 #86 의 숙달 판정("첫 시도 정답")이 뒤집힌다
