 ...
 *
 * 저장된 값의 형과 규칙(read-repair·쓰기 누적)은 `records-core.ts` 에 있다 — 거기는 `window`
 * 도 react 도 모르는 순수 층이라 node 가 부를 수 있고, CI 가 회귀 테스트를 돌린다(#214).
 * 이 파일에는 그 규칙을 localStorage·React 에 붙이는 일만 남는다. 공개 API 는 여전히 여기다.
 *
